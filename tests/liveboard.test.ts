...
describe("test liveboard mocks module", () => {
    ...
    it("checks that getItemDownloadUrl mock works as expected for success", async () => {
        await sdk.liveboard.getItemDownloadUrl('item', 1, 1).then(result => expect(result).toHaveProperty("download_url"));
    });

    it("checks that getItemDownloadUrl mock works as expected for failure", async () => {
        await sdk.liveboard.getItemDownloadUrl('item', 1, 1).catch(error => expect(error.response.data).toHaveProperty("text"));
    });
