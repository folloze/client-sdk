...
import {TargetType} from "../../src/liveboard/ILiveboardTypes";
...
describe("test liveboard mocks module", () => {
    ...
    it("trackLinkClick with missing targetType", async () => {
        await sdk.liveboard.trackLinkClick(1, {...linkClickParams, targetType: null}).catch(err => {
            expect(err.response.status).toEqual(400);
        });
    });

    it("trackLinkClick with missing url", async () => {
        await sdk.liveboard.trackLinkClick(1, {...linkClickParams, url: null}).catch(err => {
            expect(err.response.status).toEqual(400);
        });
    });
});
