...
export class Liveboard {
        ...
        return new Promise((resolve, reject) => {
            this.fetchService.fetcher
                .get(`/live_board/v2/items/${itemId}`, {
                    params: {by_slug: bySlug, board_id: boardId, with_preview_metadata: withPreviewMetadata},
                })
                .then(result => {
                    resolve(result.data);
                })
    ...
    likeItem(contentItemId: number, sourceType: string, itemId?: number): Promise<void> {
        return new Promise((resolve, reject) => {
            this.fetchService.fetcher
                .post<void>(`${this.fetchService.options.analyticsServiceEndpoint}/live_board/v1/likes`, {
                    itemId,
                    contentItemId,
                    sourceType
                })
